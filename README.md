...
- **External JSON Reading**: Automatically retrieve trigger words and sample prompts from `.metadata.json` files generated by Civitai Helper
...
`.metadata.json` file required in same folder as LoRA file:

```
/path/to/lora/
├── style_anime_v2.safetensors
├── style_anime_v2.metadata.json  ← Required
├── character_alice.safetensors
└── character_alice.metadata.json  ← Required
```

### Generating metadata.json

Auto-generated by these tools:
- [Civitai Helper](https://github.com/butaixianran/Stable-Diffusion-Webui-Civitai-Helper)
- Other LoRA management tools
...
- Confirm filename matches `{LoRAfilename}.metadata.json`
